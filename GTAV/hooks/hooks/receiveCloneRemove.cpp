...
namespace base::hooks {
	bool hooks::receiveCloneRemoveHook(CNetworkObjectMgr* mgr, CNetGamePlayer* src, CNetGamePlayer* dst, int32_t object_id, int32_t timestamp) {
		...
		return ogReceiveCloneRemoveHook(mgr, src, dst, object_id, timestamp);
	}
}