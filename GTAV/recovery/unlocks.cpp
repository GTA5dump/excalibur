...
using namespace menu::recovery::unlocks::vars;

namespace menu::recovery::unlocks::vars {
	variables m_vars;
...
namespace menu {
	...
	void recovery_unlocks_menu::render() {
		renderer::addSubmenu("Unlocks", "Recovery Unlocks", [](core* core) {
			core->addOption(buttonOption("Unlock All")
				.addClick(unlock_all));

			core->addOption(buttonOption("Tattoos")
				.addClick(unlock_tattoos));

			core->addOption(buttonOption("Clothes")
				.addClick(unlock_clothes));

			core->addOption(buttonOption("Haircuts")
				.addClick(unlock_haircuts));

			core->addOption(buttonOption("Weapons")
				.addClick(unlock_weapons));

			core->addOption(buttonOption("Camo and Parachutes")
				.addClick(unlock_camo_and_parachutes));

			core->addOption(buttonOption("Los Santos Customs")
				.addClick(unlock_los_santos_customs));

			core->addOption(buttonOption("Heist Vehicles")
				.addClick(unlock_heist_vehicles));

			core->addOption(buttonOption("Club Propularity")
				.addClick(unlock_club_popularity));

			core->addOption(buttonOption("Abilities")
				.addClick(unlock_max_abilities));

			core->addOption(buttonOption("Snacks")
				.addClick(unlock_max_snacks));

			core->addOption(buttonOption("Armor")
				.addClick(unlock_max_armor));

			core->addOption(buttonOption("Fireworks")
				.addClick(unlock_max_fireworks));

			core->addOption(buttonOption("Misc")
				.addClick(unlock_misc));

		});

	}

	void recovery_unlocks_menu::update() {
		render();
